...
class patient_window:
    def __init__(self,root) -> None:
        ...
        #create table patient(id int,name varchar(100),gender varchar(100),mobile int,visited varchar(100),address varchar(100),email varchar(100));
        ...
        self.patient_data=ttk.Treeview(data_frame,column=("id","name","gender","mobile","address","visited","email"),xscrollcommand=scrollx.set,yscrollcommand=scrolly.set)
    ...
    def update(self):
        if self.var_mobile.get()=="":
            ...
            con=mysql.connector.connect(host="localhost",username="root",password="password",database="management")
            cursor=con.cursor()
            cursor.execute("update patient set name=%s,gender=%s,mobile=%s,visited=%s,address=%s,email=%s where id=%s",(
    ...
    def search(self):
        con=mysql.connector.connect(host="localhost",username="root",password="password",database="management")
        cursor=con.cursor()
        cursor.execute("select * from patient where "+str(self.search_var.get())+" LIKE '%"+str(self.txt_search.get())+"%'")
        rows=cursor.fetchall()
        if len(rows)!=0:
            self.patient_data.delete(*self.patient_data.get_children())
            for i in rows:
