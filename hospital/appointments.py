...
## create table appointment(id int,name varchar(100),purpose varchar(100),mobile int,gender varchar(100),email varchar(100),visited varchar(100));
class appointment:
    ...
    def update(self):
        if self.var_mobile.get()=="":
            ...
            con=mysql.connector.connect(host="localhost",username="root",password="password",database="management")
            cursor=con.cursor()
            cursor.execute("update appointment set name=%s,purpose=%s,mobile=%s,gender=%s,Email=%s,Visited=%s where id=%s",(
    ...
    def search(self):
        con=mysql.connector.connect(host="localhost",username="root",password="password",database="management")
        cursor=con.cursor()
        cursor.execute("select * from appointment where "+str(self.search_var.get())+" LIKE '%"+str(self.txt_search.get())+"%'")
        rows=cursor.fetchall()
        if len(rows)!=0:
            self.patient_data.delete(*self.patient_data.get_children())
            for i in rows:
