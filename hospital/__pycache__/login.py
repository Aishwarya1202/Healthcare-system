    ...
    def forgot_password(self):
        if self.txtuser.get()=="":
            ...
            if row==None:
                ...
                lbl=Label(self.root1,text="Forgot Password",font=("times new roman",15,"bold"),fg="black",bg="gray")
                lbl.place(x=0,y=10,relwidth=1)

                securityq=Label(self.root1,text="Select Security Question",font=("times new roman",15,"bold"),bg="gray")
                securityq.place(x=50,y=80)
                self.combo_sq=ttk.Combobox(self.root1,font=("ariel",12,"bold"),width=27,state="readonly")
                self.combo_sq["value"]=("Your nickname", "Your childhood friend name","Your School name")
                self.combo_sq.place(x=50,y=110,width=250)
                self.combo_sq.current(0)
                
                securitya=Label(self.root1,text="Security Answer",font=("times new roman",15,"bold"),bg="gray")
                securitya.place(x=50,y=150)

                self.seq_entry=Entry(self.root1,font=("times new roman",15,"bold"))
                self.seq_entry.place(x=50,y=180,width=250)

                nwpwd=Label(self.root1,text="New Password",font=("times new roman",15,"bold"),bg="gray")
                nwpwd.place(x=50,y=220)

                self.newpwd_entry=Entry(self.root1,font=("times new roman",15,"bold"))
                self.newpwd_entry.place(x=50,y=250,width=250)

                btn=Button(self.root1,text="Reset",command=self.reset_pwd,font=("times new roman",15,"bold"),fg="black",bg="gray")
                btn.place(x=100,y=290)

    def reset_pwd(self):
        if self.combo_sq.get()=="":
        ...
        elif self.newpwd_entry.get()=="":
             messagebox.showerror("error","please enter password",parent=self.root1)
        else:
            con=mysql.connector.connect(host="localhost",username="root",password="password",database="login")
            cursor=con.cursor()
            query=("select * from register where email=%s and securityq=%s and securitya=%s")
            value=(self.txtuser.get(),self.combo_sq.get(),self.seq_entry.get())
            cursor.execute(query,value)
            row=cursor.fetchone()
            if row==None:
